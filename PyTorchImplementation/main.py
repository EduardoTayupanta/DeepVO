...
def train_model(model, train_loader, criterion, optimizer, epoch, batch_size, trajectory_length):
    ...
    for batch_idx, (images_stacked, odometries_stacked) in enumerate(train_loader):
        ...
        estimated_odometries = estimated_odometries.permute(1, 0, 2)
        loss = criterion(estimated_odometries[:, :, :3], odometries_stacked[:, :, :3]) + K * criterion(
            estimated_odometries[:, :, 3:], odometries_stacked[:, :, 3:])
...
def train(model, datapath, checkpoint_path, epochs, trajectory_length, args, preprocess):
    model.train()
    model.training = True

    kwargs = {'num_workers': 1, 'pin_memory': True} if torch.cuda.is_available() else {}
    train_loader = torch.utils.data.DataLoader(
        VisualOdometryDataLoader(datapath, trajectory_length=trajectory_length, transform=preprocess),
        batch_size=args.bsize, shuffle=True, drop_last=True, **kwargs)
...
def test(model, datapath, trajectory_length, validation_steps, preprocess):
    ...
    test_loader = torch.utils.data.DataLoader(
        VisualOdometryDataLoader(datapath, trajectory_length=trajectory_length, transform=preprocess, test=True),
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    if args.mode == 'train':
        train(model, args.datapath, args.checkpoint_path, args.train_iter, args.trajectory_length, args, preprocess)
    elif args.mode == 'test':
    ...
    # Data train
    # --datapath D:\EduardoTayupanta\Documentos\Librerias\dataset --trajectory_length 1 --bsize 16 --train_iter 10 --checkpoint_path D:\EduardoTayupanta\Documentos\PycharmProjects\DeepVO\PyTorchImplementation\checkpoint
    # --datapath D:\EduardoTayupanta\Documentos\Librerias\dataset --trajectory_length 1 --bsize 16 --train_iter 10 --checkpoint_path D:\EduardoTayupanta\Documentos\PycharmProjects\DeepVO\PyTorchImplementation\checkpoint --checkpoint D:\EduardoTayupanta\Documentos\PycharmProjects\DeepVO\PyTorchImplementation\checkpoint\00\checkpoint_10.pth
    # Sequence used
    # 00, 02, 05, 07, 09

    # Data test
    # --mode test --datapath D:\EduardoTayupanta\Documentos\Librerias\dataset --checkpoint D:\EduardoTayupanta\Documentos\PycharmProjects\DeepVO\PyTorchImplementation\checkpoint\00\checkpoint_10.pth